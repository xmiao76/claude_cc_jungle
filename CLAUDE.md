...
- **`ai/`** — Negamax alpha-beta search.
  - `minimax.py` — `AIPlayer` with fixed-depth (Easy/Medium) or iterative deepening (Hard). Uses quiescence search, killer-move heuristic, history heuristic, and a transposition table.
  - `evaluator.py` — Position evaluation using material + advancement + den proximity + rat-in-water bonuses.
...
| `engine/move_generator.py` | all legal moves, river-jump precomputation |
| `engine/rules.py` | capture rules, effective rank, jump blocking, win check |
| `ai/minimax.py` | negamax alpha-beta, iterative deepening, move ordering |
| `ai/evaluator.py` | static position evaluation |
