...
fn cheapest_attacker(
    ...
    let consider = |from: Square, piece: Piece, best: &mut Option<(Square, Piece, i32)>| {
        ...
        let value = PIECE_VALUES[piece.rank() as usize];
...
pub fn see(pos: &Position, mv: Move) -> i32 {
    ...
    gain[0] = PIECE_VALUES[victim.rank() as usize];
    ...
    while d + 1 < MAX_SWAPS {
        let Some((from, piece)) = cheapest_attacker(pos, to, side, removed, on_square) else {
            break;
        };
        d += 1;
        gain[d] = PIECE_VALUES[on_square.rank() as usize];
        removed |= bb(from);
        on_square = piece;
        side = side.flip();
    }
