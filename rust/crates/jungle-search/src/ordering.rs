    ...
    pub fn new(
        ...
        for (i, &m) in moves.as_slice().iter().enumerate() {
            scores[i] = if m.to() == enemy_den {
                ...
                let mvv_lva =
                    PIECE_VALUES[victim.rank() as usize] * 16 - PIECE_VALUES[attacker.rank() as usize];
                let exchange = see(pos, m);
                if exchange >= 0 {
                    GOOD_CAPTURE + mvv_lva
                } else {
...
mod tests {
    ...
    fn a_den_entry_is_ordered_first() {
        ...
        let mut om = OrderedMoves::new(&pos, generate(&pos), None, &heur, 0, None);
