...
/// Evaluate from `color`'s point of view. Higher is better for `color`.
pub fn evaluate(pos: &Position, color: Color) -> i32 {
    let mut score = side_score(pos, color) - side_score(pos, color.flip());
...
fn side_score(pos: &Position, side: Color) -> i32 {
    let si = side.index();
    ...
    for s in iter_squares(own) {
        ...
        total += PIECE_VALUES[rank as usize];

        let adv = advancement(side, s);
        total += adv * ADVANCEMENT_PER_ROW;
        if adv > MIDLINE {
            total += (adv - MIDLINE) * ADVANCEMENT_ACCELERATION;
        }
        total += PST[adv as usize][col_of(s)] * PST_WEIGHT;
        ...
        if dist <= DEN_PROXIMITY_MAX_DIST {
            total += (DEN_PROXIMITY_MAX_DIST + 1 - dist) * DEN_PROXIMITY_PER_STEP;
        }
        ...
        if rank == animal::RAT {
            if RIVER & bb(s) != 0 {
                total += RAT_IN_WATER + RAT_BLOCKS_RIVER;
            }
            if ADJACENT[s as usize] & enemy_elephant != 0 {
                total += RAT_ADJACENT_TO_ENEMY_ELEPHANT;
            }
    ...
    total += mobility as i32 * MOBILITY;

    total -= DEN_THREAT * den_threat_level(pos, side);
